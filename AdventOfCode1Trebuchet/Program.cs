...
var input = @"two65eightbkgqcsn91qxkfvg
...
for (int i = 0; i < 10; i++)
{
    validNumbers.Add(i.ToString(), i);
}
sum = 0;
foreach (var line in lines)
{
    var firstDigit = validNumbers
        .Select(x => (x, line.IndexOf(x.Key)))
        .Where(x => x.Item2 >= 0)
        .OrderBy(x => x.Item2)
        .FirstOrDefault();
    //Console.WriteLine(firstDigit);
    var lastDigit = validNumbers
        .Select(x => (x, line.LastIndexOf(x.Key)))
        .Where(x => x.Item2 >= 0)
        .OrderByDescending(x => x.Item2)
        .FirstOrDefault();
    //Console.WriteLine(lastDigit);
    var number = firstDigit.x.Value.ToString() + lastDigit.x.Value.ToString();
